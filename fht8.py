...
def fht(data_path, type):
    ...
    out_file = open('../fht8/' + type + '-head4overall.csv', 'w')

    out_file.write(",".join(range(0, 256)))
    out_file.write('\n'.join([','.join(head_4_overall[i]) for i in range(0, count_4)]))
    ...
    out_file = open('../fht8/' + type + '-trail4overall.csv', 'w')

    out_file.write(",".join(range(0, 256)))
    out_file.write('\n'.join([','.join(trail_4_overall[i]) for i in range(0, count_4)]))
